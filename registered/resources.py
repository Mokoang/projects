...
class LeaseAdminResource(resources.ModelResource):
  ...
  landuse_type      = Field(attribute='landuse_type',column_name = 'Landuse type',widget=ForeignKeyWidget(Landuse_Type, field='landuse_code'))
  lease_holder      = Field(attribute='lease_holder',column_name = 'Lease Holder')
  ...
  class Meta:
    model            = lease
    fields           = ('lease_number', 'zone_number','area','area_units','landuse_type','registration_date','lastpayment_date','tag','lease_holder','phone_number','address','district')
    export_order     = ('registration_date','lease_number', 'zone_number','area','area_units','landuse_type','lastpayment_date','tag','lease_holder','phone_number','address','district')
    exclude          = ('id','lease_status',)
    import_id_fields = ('lease_number',)
    skip_unchanged   = True
    report_skipped   = True

  def dehydrate_history_display(self, obj):
    # Map the status code to its corresponding display value
    status_mapping = dict(HISTORY)
    return status_mapping.get(obj.lease_history, obj.lease_history)
  ...
  #   if (instance.lastpayment_date != None and str(instance.lastpayment_date )!= datetime.datetime.strptime(str(instance.lastpayment_date), '%Y-%m-%d').strftime('%Y-%m-%d')) :
  #     skip = False     
  #   return skip
